...
/* PMIC charging current notifier chain */
int register_pmic_current_notifier (struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&pmic_current_notifier_chain, nb);
}

int unregister_pmic_current_notifier (struct notifier_block *nb)
{
	return atomic_notifier_chain_unregister(&pmic_current_notifier_chain, nb);
}

int pmic_current_notifier_call_chain(unsigned long val, void *v)
{
	return atomic_notifier_call_chain(&pmic_current_notifier_chain, val, v);
}
