...
#ifdef CONFIG_FALCON
static int mxc_hdmi_core_suspend(struct device *dev)
{
	return 0;
}

static int mxc_hdmi_core_resume(struct device *dev)
{
	int ret;
...
static const struct dev_pm_ops mxc_hdmi_core_pm_ops = {
	.suspend	= mxc_hdmi_core_suspend,
	.resume		= mxc_hdmi_core_resume,
};
#endif
