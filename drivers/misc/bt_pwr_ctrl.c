...
#define UART3_UCR2	0x2020084
#define UCR2_RXEN	(0x1 << 1)		/* Receiver enabled */
#define UCR2_TXEN	(0x1 << 2)		/* Transmitter enabled */
#define UCR2_WS	(0x1 << 5)		/* Word size */
#define UCR2_CTS	(0x1 << 12)		/* Clear to send */
#define UCR2_CTSC	(0x1 << 13)		/* CTS pin control */

/* uart register & prototypes */
static void __iomem *uart3_ucr2 = NULL;
static void uart_hw_flow_control_en(int en);
static void uart_hw_cts_control_en(int en) ;

/* workqueue struct */
static struct workqueue_struct *bt_pwr_ctrl_wq = 0;
...
#define bluesleep_rx_busy() do { queue_delayed_work_on(0, bt_pwr_ctrl_wq, &sleep_workqueue, 0); } while(0)

#define bluesleep_tx_busy() do { queue_delayed_work_on(0, bt_pwr_ctrl_wq, &sleep_workqueue, 0); } while(0)

#define bluesleep_rx_idle() do { queue_delayed_work_on(0, bt_pwr_ctrl_wq, &sleep_workqueue, 0); } while(0)

#define bluesleep_tx_idle() do { queue_delayed_work_on(0, bt_pwr_ctrl_wq, &sleep_workqueue, 0); } while(0)
...
/* lock for BT/UART speed switch */
static DEFINE_MUTEX(bt_uart_switch_lock);
...
/* Host High Bus Frequency Requests */
#define HOST_BUS_HIGH_RELEASE	0
#define HOST_BUS_HIGH_REQUEST	1


static inline void hostbus_high_frequency(int en)
{
	en ? request_bus_freq(BUS_FREQ_HIGH) : release_bus_freq(BUS_FREQ_HIGH);
	if (en) {
		/* checking for high bus be ready by HW */
		while (get_bus_freq_mode() != BUS_FREQ_HIGH)
			msleep(1);
	}

	uart_hw_cts_control_en(en);
...
{
	if ((gp_privdata->dev_wake == BT_WAKE_DEASSERT) &&
...
static void bluesleep_sleep_wakeup(void)
{
	if (test_bit(BT_ASLEEP, &gp_privdata->state)) {
		/* Request Host High Bus Freq */
		hostbus_high_frequency(HOST_BUS_HIGH_REQUEST);
...
static void bluesleep_work(struct work_struct *work)
{
	...
	if (bluesleep_can_sleep(gp_privdata->pdata)) {
		...
		/* Release Host High Bus Freq */
		hostbus_high_frequency(HOST_BUS_HIGH_RELEASE);
	} else {
...
static void bluesleep_tx_timer_expire(unsigned long unused)
{
	...
	if (!test_bit(BT_TXDATA, &gp_privdata->state)) {
		...
		gp_privdata->dev_wake = BT_WAKE_DEASSERT;
...
static int bluesleep_start(void)
{
	...
	gp_privdata->dev_wake = BT_WAKE_ASSERT;
	...
	printk(KERN_INFO "Bluesleep hw flow control start\n");
	uart_hw_flow_control_en(1);
...
static void bluesleep_stop(void)
{
	...
	msleep(10);	/* wait for taskets to finish */
	if (!test_bit(BT_ASLEEP, &gp_privdata->state)) {
		set_bit(BT_ASLEEP, &gp_privdata->state);
		/* Release Host High Bus Freq */
		hostbus_high_frequency(HOST_BUS_HIGH_RELEASE);
	...
	printk(KERN_INFO "Bluesleep hw flow control stop\n");
	uart_hw_flow_control_en(0);
}
...
static void uart_hw_flow_control_en(int en) {
	...
	reg = readl(uart3_ucr2);

	if (en) {
		reg |= (UCR2_RXEN | UCR2_TXEN | UCR2_WS |UCR2_CTSC) ;
	} else {
		reg &= ~(UCR2_RXEN | UCR2_TXEN | UCR2_WS | UCR2_CTSC);
	}

	writel(reg, uart3_ucr2);
}
...
ssize_t read_proc_devwake(struct file *filp, char *buf, size_t count, loff_t *offp)
{
	...
	spin_lock_irqsave(&rw_lock, irq_flags);
	devwake = gp_privdata->dev_wake ? '1' : '0';
	spin_unlock_irqrestore(&rw_lock, irq_flags);
		...
		loff_t *offp)
{
	...
	if(1 == sscanf(buf, " %d", &on)) {
		mutex_lock(&bt_uart_switch_lock);
		uart_sleep_and_switch_parent(!!on);
		mutex_unlock(&bt_uart_switch_lock);
	}
...
static int bt_pwrctrl_probe(struct platform_device *pdev)
{
	...
	if(!pdata) {
		...
		retval = devm_gpio_request_one(&pdev->dev, pdata->bt_dev_wake, GPIOF_OUT_INIT_HIGH, "BT_DEV_WAKE");
		if(retval) {
	...
	IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "BT hostwake", NULL);
	if(retval < 0) {
	...
	/* create bt power workqueue */
	bt_pwr_ctrl_wq = alloc_ordered_workqueue("btpowercontrol", WQ_MEM_RECLAIM | WQ_HIGHPRI,
							"btpowercontrolworkqueue");
	if (!bt_pwr_ctrl_wq) {
	...
	/* trigger ext BT asleep */
	gp_privdata->dev_wake = BT_WAKE_DEASSERT;
	gpio_set_value(gp_privdata->pdata->bt_dev_wake, BT_WAKE_DEASSERT);
	gp_privdata->state = 0;
	set_bit(BT_ASLEEP, &gp_privdata->state);

	/* ioremap uart3_ucr2 register */
	uart3_ucr2 = ioremap(UART3_UCR2, 1);
...
static int bt_pwrctrl_remove(struct platform_device *pdev)
{
	...
	if (bt_pwr_ctrl_wq)
		destroy_workqueue(bt_pwr_ctrl_wq);
