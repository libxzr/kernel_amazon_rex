...
static irqreturn_t imx_int(int irq, void *dev_id)
{
...
#ifdef CONFIG_LAB126
	/* When BT is turned off, we switch uart clk source back to low speed clock to save power.
	 * The serial_imx_resume resume both console uart and BT uart.
	 * Therefore if there are linger data activities on the BT uart bus, it would trigger interrupt when BT is actually off.
	 * This would cause watchdog. Therefore it is necessary to ignore the interrupt from BT when BT is off.
	 */
	...
	if (sts2 & USR2_ORE) {
		/*
		 *This is debug_level messages will not appear on the serial console.
		 */
		dev_printk(KERN_DEBUG, sport->port.dev,"Rx FIFO overrun\n");
		sport->port.icount.overrun++;
...
static void imx_release_port(struct uart_port *u)
{
}

static int imx_request_port(struct uart_port *u)
{
	return 0;
}
...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	if (txirq > 0) {
		...
		if (ret)
			return ret;
#ifdef CONFIG_LAB126
		if (strcmp("2034000.serial", dev_name(&pdev->dev)) == 0) {
			g_BTirq = sport->port.irq;
			dev_dbg(&pdev->dev, "g_BTirq:%d\n", g_BTirq);
		}
#endif
	}
