...
struct ci_hdrc_imx_data {
	...
	struct clk *clk_per;
	/* --------------------------------- */
	/* Since the BUS_FREQ request and release are reference counted, so add flag to make sure they are matched */
#define HIGH_BUS_FREQ	1
...
int imx_ci_hdrc_enable_clks(struct ci_hdrc *ci)
{
	struct device *dev = ci->dev->parent;
	return imx_prepare_enable_clks(dev);
}
EXPORT_SYMBOL(imx_ci_hdrc_enable_clks);
...
static int ci_hdrc_imx_probe(struct platform_device *pdev)
{
	...
	set_bit(HIGH_BUS_FREQ, &data->bitflags);
	...
	clear_bit(HIGH_BUS_FREQ, &data->bitflags);
...
static int ci_hdrc_imx_remove(struct platform_device *pdev)
{
	...
	ci_hdrc_remove_device(data->ci_pdev);
	imx_disable_unprepare_clks(&pdev->dev);
	if (test_and_clear_bit(HIGH_BUS_FREQ, &data->bitflags)) {
		release_bus_freq(BUS_FREQ_HIGH);
	}
	if (data->hsic_pad_regulator)
...
static int imx_controller_resume(struct device *dev)
{
	...
	if (!test_and_set_bit(HIGH_BUS_FREQ, &data->bitflags)) {
		request_bus_freq(BUS_FREQ_HIGH);
	}
