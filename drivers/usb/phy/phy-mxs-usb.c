...
#define HW_USBPHY_TX_CAL_45DP_MASK		0x0f0000
#define HW_USBPHY_TX_CAL_45DN_MASK		0x000f00
#define HW_USBPHY_TX_D_CAL_MASK			0x00000f
#define HW_USBPHY_TX_CAL_INVALID		0xffffff
...
static int mxs_phy_hw_init(struct mxs_phy *mxs_phy)
{
...
#ifdef CONFIG_USB_REX
	if (mxs_phy->phy_3p0 && !mxs_phy->vcc_enabled) {
	...
	/* Change 45DP, 45DN, D_CAL if necessary, those values can be zero on purpose */
	new_val = val = readl(base + HW_USBPHY_TX);
	if (mxs_phy->tx_cal_45dp != HW_USBPHY_TX_CAL_INVALID) {
		new_val &= ~HW_USBPHY_TX_CAL_45DP_MASK;
		new_val |= mxs_phy->tx_cal_45dp << 16;
	}
	if (mxs_phy->tx_cal_45dn != HW_USBPHY_TX_CAL_INVALID) {
		new_val &= ~HW_USBPHY_TX_CAL_45DN_MASK;
		new_val |= mxs_phy->tx_cal_45dn << 8;
	}
	if (mxs_phy->tx_d_cal != HW_USBPHY_TX_CAL_INVALID) {
		new_val &= ~HW_USBPHY_TX_D_CAL_MASK;
		new_val |= mxs_phy->tx_d_cal;
	}
	if (new_val != val) {
		writel(new_val, base + HW_USBPHY_TX);
	}
...
#ifdef CONFIG_USB_REX
	if (mxs_phy->phy_3p0 && mxs_phy->vcc_enabled) {
		if (regulator_disable(mxs_phy->phy_3p0))
			dev_err(mxs_phy->phy.dev, "Failed to disable 3p0 regulator\n");
		else
			mxs_phy->vcc_enabled = false;
	}
#else
	if (mxs_phy->phy_3p0)
...
#ifdef CONFIG_USB_REX
static void mxs_phy_set_vcc(struct usb_phy *phy, int on)
{
	struct mxs_phy *mxs_phy = to_mxs_phy(phy);

	if (mxs_phy->phy_3p0) {
		if (on && !mxs_phy->vcc_enabled) {
			if (regulator_enable(mxs_phy->phy_3p0))
				dev_err(mxs_phy->phy.dev, "Failed to enable 3p0 regulator\n");
			else
				mxs_phy->vcc_enabled = true;
			dev_dbg(mxs_phy->phy.dev, "3p0 regulator enabled\n");
		}
		else if (!on && mxs_phy->vcc_enabled) {
			if (regulator_disable(mxs_phy->phy_3p0))
				dev_err(mxs_phy->phy.dev, "Failed to disable 3p0 regulator\n");
			else
				mxs_phy->vcc_enabled = false;
			dev_dbg(mxs_phy->phy.dev, "3p0 regulator disabled\n");
		}
	}
}

static int mxs_phy_get_vcc(struct usb_phy *phy)
{
	struct mxs_phy *mxs_phy = to_mxs_phy(phy);

	return mxs_phy->vcc_enabled;
}
#endif
...
static int mxs_phy_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_USB_REX
	mxs_phy->phy.set_vcc	= mxs_phy_set_vcc;
	mxs_phy->phy.get_vcc	= mxs_phy_get_vcc;
#endif
	...
	if (mxs_phy->phy_3p0)
#ifdef CONFIG_LAB126
		regulator_set_voltage(mxs_phy->phy_3p0, 3000000, 3000000);
#else
	...
	mxs_phy->tx_cal_45dp = HW_USBPHY_TX_CAL_INVALID;
	if (of_find_property(np, "tx-cal-45dp", NULL)) {
		ret = of_property_read_u32(np, "tx-cal-45dp",
			&mxs_phy->tx_cal_45dp);
		if (ret) {
			dev_err(&pdev->dev,
				"failed to get tx-cal-45dp value\n");
			return ret;
		}
	}

	mxs_phy->tx_cal_45dn = HW_USBPHY_TX_CAL_INVALID;
	if (of_find_property(np, "tx-cal-45dn", NULL)) {
		ret = of_property_read_u32(np, "tx-cal-45dn",
			&mxs_phy->tx_cal_45dn);
		if (ret) {
			dev_err(&pdev->dev,
				"failed to get tx-cal-45dn value\n");
			return ret;
		}
	}

	mxs_phy->tx_d_cal = HW_USBPHY_TX_CAL_INVALID;
