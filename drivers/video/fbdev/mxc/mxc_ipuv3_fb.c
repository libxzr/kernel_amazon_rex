...
#ifdef CONFIG_FALCON
		if (in_falcon() && g_dp_in_use[mxc_fbi->ipu_id]) {
			ipu_clear_irq(mxc_fbi->ipu, mxc_fbi->ipu_ch_irq);
			ipu_disable_irq(mxc_fbi->ipu, mxc_fbi->ipu_ch_irq);

			if (mxc_fbi->ipu_alp_ch_irq != -1) {
				ipu_clear_irq(mxc_fbi->ipu,
					      mxc_fbi->ipu_alp_ch_irq);
				ipu_disable_irq(mxc_fbi->ipu,
						mxc_fbi->ipu_alp_ch_irq);
			}
		}
#endif /* CONFIG_FALCON */
...
static int mxcfb_resume(struct platform_device *pdev)
{
	...
	if (in_falcon()) {
		if (pdev->id == 0) {
			ipu_disp_set_global_alpha(mxc_fbi->ipu, mxc_fbi->ipu_ch,
					mxc_fbi->save.global_alpha.enable,
					mxc_fbi->save.global_alpha.alpha);

			ipu_disp_set_color_key(mxc_fbi->ipu, mxc_fbi->ipu_ch,
					mxc_fbi->save.key.enable,
					mxc_fbi->save.key.color_key);
		}

		if (g_dp_in_use[mxc_fbi->ipu_id]) {
			ipu_enable_irq(mxc_fbi->ipu, mxc_fbi->ipu_ch_irq);
			if (mxc_fbi->ipu_alp_ch_irq != -1)
				ipu_enable_irq(mxc_fbi->ipu,
					       mxc_fbi->ipu_alp_ch_irq);
		}
	}
#endif /* CONFIG_FALCON */
...
#ifdef CONFIG_FALCON
		mxcfbi->save.global_alpha.enable = true;
		mxcfbi->save.global_alpha.alpha = 0x80;
		mxcfbi->save.key.enable = false;
#endif
