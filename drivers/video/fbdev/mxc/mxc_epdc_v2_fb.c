...
static inline void epdc_force_used_luts_to_hw_val(void)
{
	used_luts = (u64)__raw_readl(EPDC_STATUS_LUTS) |
					(u64)__raw_readl(EPDC_STATUS_LUTS2) << 32;
}
#ifdef EPDC_STANDARD_MODE
/*
...
static int epdc_choose_next_lut(struct mxc_epdc_fb_data *fb_data, int *next_lut)
{
	...
	while (!epdc_any_luts_available()) {
		...
		/* Give chance let fb_data->luts_complete get update. */
		if(mutex_is_locked(&fb_data->queue_mutex))
		{
		...
		if(++cnt > 3)
		{
			printk(KERN_ERR "EPDC_STATUS_LUTS = 0x%x, EPDC_STATUS_LUTS2 = 0x%x\n",
				__raw_readl(EPDC_STATUS_LUTS),
				__raw_readl(EPDC_STATUS_LUTS2));
			printk(KERN_ERR "available luts lost, force used_luts to hardware val.\n");
			epdc_force_used_luts_to_hw_val();
		}
	}
