					...
					unsigned long val, void *v)
{
...
#ifdef CONFIG_FALCON
		if ((hdmi->blank == FB_BLANK_POWERDOWN && in_falcon()) ||
			(hdmi->blank == FB_BLANK_UNBLANK)) {
#else
	...
	case FB_EVENT_RESUME:
		...
		if (hdmi->blank == FB_BLANK_UNBLANK) {
...
#ifdef CONFIG_FALCON
		else if (hdmi->blank == FB_BLANK_POWERDOWN && in_falcon()) {
			/* mipi_core, hdmi_iahb, hdmi_isfr clock is enabled. */
			mxc_hdmi_phy_init(hdmi);
			...
			/* Unmute interrupts */
			hdmi_writeb(~HDMI_IH_MUTE_PHY_STAT0_HPD,
...
#ifdef CONFIG_FALCON
static int mxc_hdmi_suspend(struct device *dev)
{
	struct mxc_hdmi *hdmi = dev_get_drvdata(dev);
...
static int mxc_hdmi_resume(struct device *dev)
{
	return 0;
}

static const struct dev_pm_ops mxc_hdmi_pm_ops = {
	.suspend = mxc_hdmi_suspend,
	.resume = mxc_hdmi_resume,
};
#endif
