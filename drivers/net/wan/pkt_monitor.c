...
static int pkt_mon_doit(struct sk_buff *, struct genl_info *);

static const struct genl_ops pkt_mon_gnl_ops_doit[] = {
	{
		.cmd = PKT_MON_CMD,
		.flags = 0,
...
static u32                saved_pid ;
static struct nf_hook_ops nfho;
static volatile int       monitor_enabled = 0 ;
static unsigned long      monitor_timeout = 0 ;

static void send_mesg_to_user_app(void)
{
    struct sk_buff *skb;
    int rc;
    void *msg_head;
...
fail:
	pr_err("pkt_mon: Could not send message\n");
	return;
}
 ...
 */
static int pkt_mon_doit(struct sk_buff *skb_2, struct genl_info *info)
{
    struct nlattr *na;
    tph_info *tinfo;
...
static int __init pkt_mon_init(void)
{
    ...
    /*register new family with functions (commands) */
    rc = genl_register_family_with_ops(&pkt_mon_gnl_family, pkt_mon_gnl_ops_doit);
    if (rc != 0){
