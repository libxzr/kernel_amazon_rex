 ...
 *
 * $Id: dhd_common.c 626588 2016-03-22 06:54:57Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#if defined(PKT_FILTER_SUPPORT) || defined(AMZN_EREADER_PKT_FILTER_SUPPORT)
...
#endif /* PKT_FILTER_SUPPORT || AMZN_EREADER_PKT_FILTER_SUPPORT */
...
wl_pkt_filter_base_list(void)
{
	...
	DHD_ERROR(("Names accepted for base offsets:\n"));
...
static int
dhd_pkt_filter_add(dhd_pub_t * dhd, char **argv, char *buf)
{
	const char          *str;
	wl_pkt_filter_t     pkt_filter;
	wl_pkt_filter_t     *pkt_filterp;
	...
	if (*endptr) {
		DHD_ERROR(("Invalid number for id: %s\n", *argv));
		return BCME_USAGE_ERROR;
	}
	...
	if (*endptr) {
		DHD_ERROR(("Invalid number for polarity: %s\n", *argv));
		return BCME_USAGE_ERROR;
	}
	...
	if (*endptr) {
		DHD_ERROR(("Invalid number for filter type: %s\n", *argv));
		return BCME_USAGE_ERROR;
	}
	...
	if ((ftype == WL_PKT_FILTER_TYPE_PATTERN_MATCH) ||
		...
		if (*endptr) {
			DHD_ERROR(("Invalid number for offset: %s\n", *argv));
			return BCME_USAGE_ERROR;
		}
	...
	if (ftype == WL_PKT_FILTER_TYPE_APF_MATCH) {
		...
		if (strlen(token) % 2) {
			DHD_ERROR(("Invalid APF program length: %s\n", *argv));
			return BCME_USAGE_ERROR;
		}
	...
	if (ftype == WL_PKT_FILTER_TYPE_PATTERN_MATCH_TIMEOUT) {
		...
		if (*++argv == NULL) {
			DHD_ERROR(("timeout not specified\n"));
			return BCME_USAGE_ERROR;
		}
...
void
dhd_amzn_ereader_pktfilter_set(dhd_pub_t * dhd, char *arg)
{
	char	*argv[64];
	int		i = 0;
	char	*arg_save = 0, *arg_org = 0;
...
#define BUF_SIZE		2048
	...
	dhd_pkt_filter_add(dhd, &argv[0], buf);
