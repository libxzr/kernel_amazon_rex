 ...
 */
/*
 * Copyright (c) 2016-2017 Amazon.com, Inc. or its affiliates.
 * All Rights Reserved.
 */
...
#if defined(AMZN_EREADER_PKT_FILTER_SUPPORT)
extern void dhd_amzn_ereader_pktfilter_set(dhd_pub_t * dhd, char *arg);
#endif /* AMZN_EREADER_PKT_FILTER_SUPPORT && !PKT_FILTER_SUPPORT */
 ...
 */
#ifdef CONFIG_LAB126
#define BRCM_4343W_FIRMWARE "/opt/brcm_4343w/wifi/firmware/fw_bcmdhd_sr.bin"
#define BRCM_4343W_NVRAM "/opt/brcm_4343w/wifi/nvram/nvram.txt"
...
/* Set frameburst to value */
	...
	/* Setup filter to allow only unicast */
	dhd->pktfilter[DHD_AMZN_EREADER_BROADCAST_FILTER_NUM]
		= "110 0 2 "
		  "START:0 0xffffffffffff 0xffffffffffff "
		  "START:12 0xffffff 0x080045 "
		  "IP4_H:9 0xff 0x11 "
		  "IP4_H:16 0xffffffff 0xffffffff "
		  "IP4_H:28 0xffffffffffffffff 0x4b414e4e4f55254e";
	dhd->pktfilter[DHD_AMZN_EREADER_ARP_FILTER_NUM]
		= "111 0 0 12 0xFFFF 0x0806";

	{
		uint32 amzn_ereader_pkt_filter_mode = 0;
		wl_pkt_filter_enable_t	amzn_ereader_pkt_filter_enable;
		int32 filter_num;
		/* Add pkt filter  */
		for (filter_num = 0; filter_num < dhd->pktfilter_count; filter_num++) {
			dhd_amzn_ereader_pktfilter_set(dhd, dhd->pktfilter[filter_num]);
		}

		/* Set pkt filter mode as 0  */
		if ((ret = dhd_iovar(dhd, 0, "pkt_filter_mode", (char *)&amzn_ereader_pkt_filter_mode,
			sizeof(amzn_ereader_pkt_filter_mode), NULL, 0, TRUE)) < 0) {
			DHD_ERROR(("%s Set pkt_filter_mode 0 failed  %d\n",
				__FUNCTION__, ret));
		}

		/* Enable pkt_filter ID:110 */
		amzn_ereader_pkt_filter_enable.id = 110;
		amzn_ereader_pkt_filter_enable.enable = 1;
		if ((ret = dhd_iovar(dhd, 0, "pkt_filter_enable", (char *)&amzn_ereader_pkt_filter_enable,
			sizeof(amzn_ereader_pkt_filter_enable), NULL, 0, TRUE)) < 0) {
			DHD_ERROR(("%s Enable pkt filter ID %d failed  %d\n",
				__FUNCTION__, amzn_ereader_pkt_filter_enable.id, ret));
		}

	}
#endif /* AMZN_EREADER_PKT_FILTER_SUPPORT */
		...
		uint res_len, int set)
{
	...
	DHD_TRACE(("%s:name %s/set %d\n", __FUNCTION__, name, set));
...
#ifdef CONFIG_LAB126
	fw_image_unaligned = fw_image = MALLOC(NULL, FW_IMAGE_MAX + DHD_SDALIGN);
	if (!fw_image) {
		DHD_ERROR(("%s: Failed to allocate fw_image memory %d bytes\n", __func__, FW_IMAGE_MAX));
		return -ENOMEM;
	}
