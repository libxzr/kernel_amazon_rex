 ...
 */
/*
 * Copyright (c) 2016-2018 Amazon.com, Inc. or its affiliates.
 * All Rights Reserved.
 */
...
#define HOME_SCREEN_DELAY	4000	/* delay 4s for Home screen, see JIRA: JSEVENONE-2172, JSEVENONE-5410 */
static void bcmsdh_sdmmc_resume_worker(struct work_struct *work);
#if defined(CONFIG_TOI)
extern u8 is_hibernation, is_hibernation_thaw;
#endif
#ifdef CONFIG_FALCON
...
#ifdef CONFIG_LAB126
	INIT_DELAYED_WORK(&sdioh->bcmsdh_work, bcmsdh_sdmmc_resume_worker);
#endif /* CONFIG_LAB126 */
...
#ifdef CONFIG_LAB126
	cancel_delayed_work_sync(&sdioh->bcmsdh_work);
#endif /* CONFIG_LAB126 */
}
...
#ifdef CONFIG_LAB126
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
static int _bcmsdh_sdmmc_suspend_late(struct device *pdev, char *f)
{
	struct sdio_func *func = dev_to_sdio_func(pdev);

	pr_info("%s: pdev=%p FN:%d\n", f, pdev, func ? func->num: -1);
...
{
	return _bcmsdh_sdmmc_suspend_late(pdev, __func__);
}

static int bcmsdh_sdmmc_freeze_late(struct device *pdev)
{
	return _bcmsdh_sdmmc_suspend_late(pdev, __func__);
}

static int _bcmsdh_sdmmc_resume_early(struct device *pdev, char *f)
{
	struct sdio_func *func = dev_to_sdio_func(pdev);
...
{
	return _bcmsdh_sdmmc_resume_early(pdev, __func__);
}

static int bcmsdh_sdmmc_restore_early(struct device *pdev)
{
	return _bcmsdh_sdmmc_resume_early(pdev, __func__);
}

static int bcmsdh_sdmmc_thaw_early(struct device *pdev)
{
	return _bcmsdh_sdmmc_resume_early(pdev, __func__);
}
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0) */
...
static void bcmsdh_sdmmc_resume_worker(struct work_struct *work)
{
	sdioh_info_t *sdioh = container_of(work, struct sdioh_info, bcmsdh_work.work);
	int ret;

	if (!sdioh || !sdioh->func || !sdioh->func[2]) {
		pr_err("%s: sdioh or F2 is NULL\n", __func__);
		return;
	}

	ret = sdioh_probe(sdioh->func[2]);
	if (ret) {
		pr_err("%s: sdioh_probe, ret :%d\n", __func__, ret);
		return;
	}
	ret = sdioh_start(sdioh, 0);
	if (ret) {
		pr_err("%s: sdioh_start, ret: %d\n", __func__, ret);
		return;
	}
}
#endif /* CONFIG_LAB126 */
...
#ifdef CONFIG_LAB126
#ifdef CONFIG_TOI
	need_delay |= (is_hibernation && !is_hibernation_thaw);
#endif
#ifdef CONFIG_FALCON
	need_delay |= in_falcon();
#endif
	if (need_delay) {
		/* Schedule driver init to a point right after Home screen
		 * has been displayed.
		 */
		schedule_delayed_work(&sdioh->bcmsdh_work,
				      msecs_to_jiffies(HOME_SCREEN_DELAY));
		return 0;
	}
	ret = sdioh_probe(func);
	if (ret) {
...
static const struct dev_pm_ops bcmsdh_sdmmc_pm_ops = {
	...
	.freeze_late	= bcmsdh_sdmmc_freeze_late,
	.restore_early	= bcmsdh_sdmmc_restore_early,
	.thaw_early	= bcmsdh_sdmmc_thaw_early,
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0) */
#endif /* CONFIG_LAB126 */
};
