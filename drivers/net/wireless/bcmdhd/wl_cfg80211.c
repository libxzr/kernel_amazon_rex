 ...
 *
 * $Id: wl_cfg80211.c 630415 2016-04-08 21:30:10Z $
 */
/*
...
#ifdef CONFIG_LAB126
static const char *wl_print_ssid(char *buf, const char *ssid, int ssid_len)
{
	int i;

	ssid_len = min_t(int, ssid_len, IEEE80211_MAX_SSID_LEN);
	buf[0] = (ssid_len > 2) ? ssid[0] : '*';
	for (i = 1; i < ssid_len - 1; i++)
		buf[i] = '*';
	buf[ssid_len - 1] = (ssid_len > 2) ? ssid[ssid_len - 1] : '*';
	buf[ssid_len] = '\0';
	...
	struct cfg80211_connect_params *sme)
{
	...
	*/
	...
	if (cfg->rcc_enabled) {
#ifdef CONFIG_LAB126
		pr_info("Connecting with " MACDBG " ssid \"%s\" len (%d) with rcc channels \n\n",
			MAC2STRDBG((u8*)(&ext_join_params->assoc.bssid)),
			wl_print_ssid(buf, ext_join_params->ssid.SSID, ext_join_params->ssid.SSID_len),
			ext_join_params->ssid.SSID_len);
#else
...
#ifdef CONFIG_LAB126
		pr_info("Connecting with " MACDBG " ssid \"%s\" len (%d) channel=%d\n\n",
			MAC2STRDBG((u8*)(&ext_join_params->assoc.bssid)),
			wl_print_ssid(buf, ext_join_params->ssid.SSID, ext_join_params->ssid.SSID_len),
...
extern volatile bool dhd_mmc_suspend;
	if (dhd_mmc_suspend) {
		pr_info("%s:%d %s: bus is in suspend\n",
...
static s32 wl_setup_wiphy(struct wireless_dev *wdev, struct device *sdiofunc_dev, void *context)
{
	...
	if (brcm_wowlan_config) {
		...
		brcm_wowlan_config->nd_config = 0;
	} else {
...
static s32 wl_get_assoc_ies(struct bcm_cfg80211 *cfg, struct net_device *ndev)
{
	...
	if (assoc_info.req_len) {
		...
		if (conn_info->req_ie_len <= MAX_REQ_LINE)
			...
			WL_ERR(("REQ IE size %d above max %d size \n",
	...
	if (assoc_info.resp_len) {
		...
		if (conn_info->resp_ie_len <= MAX_REQ_LINE) {
			...
			WL_ERR(("RESP IE size %d above max %d size \n",
...
wl_get_assoc_ies_ret:
	mutex_unlock(&cfg->ioctl_buf_sync);
	return err;
}
...
#ifdef CONFIG_LAB126
	if (cfg && cfg->event_tsk.thr_pid > 0) {
		WL_INFORM(("reuse thread pid %ld\n", cfg->event_tsk.thr_pid));
		return 0;
	}
#endif /* CONFIG_LAB126 */
...
{
#ifdef CONFIG_LAB126
	/* FW doesn't support GTK offload */
	return BCME_UNSUPPORTED;
#else /* !CONFIG_LAB126 */
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	s32 err = 0;
