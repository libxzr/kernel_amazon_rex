...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	if (!fw_image_size) {
		printk("%s @%lu: read fw to %p\n", __func__, jiffies, fw_image);

		image = dhd_os_open_image(pfw_path);
		if (image == NULL)
			goto err;
	} else {
		printk("%s @%lu: copy fw from %p\n", __func__, jiffies, fw_image);
	}
#else /* !CONFIG_LAB126 */
	image = dhd_os_open_image(pfw_path);
	if (image == NULL)
	...
	printk("%s @%lu: finished download fw_image_size=%d\n", __func__, jiffies, fw_image_size);
#endif /* CONFIG_LAB126 */
