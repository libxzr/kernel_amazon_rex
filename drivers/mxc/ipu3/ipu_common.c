...
static int ipu_resume(struct device *dev)
{
	...
	if (in_falcon()) {
		...
		if (!ipu->bypass_reset) {
			ret = device_reset(ipu->dev);
			if (ret) {
				dev_err(ipu->dev, "failed to reset: %d\n", ret);
				return ret;
			}
			...
			/* Set MCU_T to divide MCU access window into 2 */
			ipu_cm_write(ipu,
				     0x00400000L | (IPU_MCU_T_DEFAULT << 18),
				     IPU_DISP_GEN);

			ret = clk_set_parent(ipu->pixel_clk_sel[0],
					     ipu->ipu_clk);
			if (ret) {
			...
			ret = clk_set_parent(ipu->pixel_clk_sel[1],
					     ipu->ipu_clk);
			if (ret) {
				dev_err(ipu->dev, "clk set parent failed");
				return ret;
			...
			/* Set sync refresh channels and CSI->mem channel as
			   high priority */
...
#ifdef CONFIG_FALCON
	SET_SYSTEM_SLEEP_PM_OPS(NULL, ipu_resume)
#endif
};
#endif
