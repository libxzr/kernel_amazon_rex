...
static unsigned long   bitflags;

//#define CONFIG_LAB126_CPUFREQ
#ifdef CONFIG_LAB126_CPUFREQ
#define READ_OPERNATION 2
static unsigned char   power_optimization = true;
unsigned char imx6q_cpufreq_power_optimization(unsigned char enable_optimization)
{
	mutex_lock(&set_cpufreq_lock);
	if (enable_optimization < READ_OPERNATION)
		power_optimization = enable_optimization;
	mutex_unlock(&set_cpufreq_lock);

	return power_optimization;
}
EXPORT_SYMBOL_GPL(imx6q_cpufreq_power_optimization);
#endif
static int imx6q_set_target(struct cpufreq_policy *policy, unsigned int index)
{
	 ...
	 */
#ifdef CONFIG_LAB126_CPUFREQ
    /* 2 cpu freq -- 0 (396Mhz), 1 (996 Mhz) */
    if (power_optimization == false)
        goto _no_power_optimization;
	if (new_freq == freq_table[1].frequency) {
	    if (!test_and_set_bit(0, &bitflags))
...
#ifdef CONFIG_LAB126_CPUFREQ
    /* Release bus if cpu is not set to 996 Mhz */
	else {
	    if (test_and_clear_bit(0, &bitflags))
...
void imx6q_cpufreq_max(void)
{
    pr_info("%s: pull up cpu/bus freq\n", __func__);
    imx6q_set_target(cpufreq_cpu_get(0), 1);
}
EXPORT_SYMBOL_GPL(imx6q_cpufreq_max);
#endif
