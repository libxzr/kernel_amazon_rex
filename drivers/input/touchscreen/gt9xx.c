...
static ssize_t gtp_touch_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gtp_touch_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *gtp_touch_proc_entry = NULL;
...
static ssize_t gtp_misc_write_not_implemented(struct file *file, const char __user *buf,
			size_t count, loff_t *pos);
static ssize_t gtp_misc_read_not_implemented(struct file *file, char __user *buf,
			size_t count, loff_t *pos);
static long gtp_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
    ...
    if (ret < 0) {
        ...
        if (temp_data[5] == 0x00)
            ptr += sprintf(ptr, "PID: %c%c%c VID:%02x%02x\n",
                           temp_data[2], temp_data[3], temp_data[4], temp_data[7], temp_data[6]);
        else
	    ptr += sprintf(ptr, "PID: %c%c%c%c VID:%02x%02x\n",
                           temp_data[2], temp_data[3], temp_data[4], temp_data[5], temp_data[7], temp_data[6]);
    }
			...
			size_t len, loff_t *off)
{
	...
	if (copy_from_user(command, buf, len)) {
		GTP_ERROR("gtp_touch_write_proc:proc::cannot copy from user!\n");
		return -EFAULT;
	}

	if ( !strncmp(command, "unlock", 6) ) {
		printk(KERN_INFO "gtp: gtp_touch_write_proc::command=unlock:\n");
		goodix_ts_resume(ts);
		ts->exlock = false;
	} else if ( !strncmp(command, "lock", 4) ) {
		printk(KERN_INFO "gtp: gtp_touch_write_proc::command=lock:\n");
		goodix_ts_suspend(ts);
		ts->exlock = true;
	} else {
	        GTP_ERROR("gtp_touch_write_proc:proc:command=%s:Unrecognized command\n", command);
	}
...
static long gtp_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case TOUCH_IOCTL_SET_LOCK:
		{
			...
			if (locked) {
				printk(KERN_INFO "gtp: gtp_touch_write_ioctl::command=lock:\n");
				goodix_ts_suspend(ts);
				ts->exlock = true;
			} else {
				printk(KERN_INFO "gtp: gtp_touch_write_ioctl::command=unlock:\n");
				goodix_ts_resume(ts);
				ts->exlock = false;
			}
			break;
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    gtp_touch_proc_entry = proc_create(GTP_TOUCH_PROC_FILE, 0666, NULL, &gtp_touch_fops);
