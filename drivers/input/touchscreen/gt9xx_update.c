...
// The correct conditions:
//  1. the config version in update file is greater than the config version in ic
/* Update Conditions:
    1. File CFG Ver > IC CFG Ver
*/

static u8 gup_enter_update_config_judge(struct i2c_client *client, u8 *file_data)
{
    u8  rd_buf[5];
    s32 ret = 0;
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	...
	if (SEEK_SET == set_or_end) {
		if (update_msg.fw->size < FW_HEAD_LENGTH + offset + length) {
			GTP_ERROR("No enough firmware data for read");
			return -EFAULT;
		}
		memcpy(buf, &update_msg.fw->data[FW_HEAD_LENGTH + offset],
		       length);
	} else {
		memcpy(buf, &update_msg.fw->data[update_msg.fw->size - offset],
		       length);
        }
