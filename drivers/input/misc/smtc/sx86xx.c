...
#ifdef CONFIG_LAB126
static void sx86xx_late_init_worker_func(struct work_struct *work)
{
  psx86XX_t this = 0;

  if (work) {
    this = container_of(work,sx86XX_t,dinit.work);

    if (!this) {
      printk(KERN_ERR "sx86xx_late_init_worker_func, NULL sx86XX_t\n");
      return;
    }
...
void sx86XX_resume(psx86XX_t this)
{
  ...
  sx86XX_schedule_work(this,0);
    ...
    enable_irq(this->irq);
#else
  /* Do the init in a delayed workqueue to speed up the resume time */
  schedule_delayed_work(&this->dinit, msecs_to_jiffies(SX86XX_LATE_RESUME_INIT_DELAY));
#endif
  }
}
...
#ifdef CONFIG_LAB126
    /* initialize worker function */
	  INIT_DELAYED_WORK(&this->dinit, sx86xx_late_init_worker_func);
#endif
...
#ifdef CONFIG_LAB126
    cancel_delayed_work_sync(&this->dinit); /* Cancel the init Worker Func */
#endif
#ifdef CONFIG_HAS_WAKELOCK
