...
#ifdef CONFIG_FALCON
		if (host->adma_table)
			falcon_set_preload(host->adma_addr,
					   PAGE_ALIGN(host->adma_table_sz) >> PAGE_SHIFT);
		if (host->align_buffer)
			falcon_set_preload(virt_to_phys(host->align_buffer),
					   PAGE_ALIGN(host->align_buffer_sz) >> PAGE_SHIFT);
#endif
	}
...
#ifdef CONFIG_FALCON
	if (host->adma_table)
		falcon_clr_preload(host->adma_addr,
				   PAGE_ALIGN(host->adma_table_sz) >> PAGE_SHIFT);
	if (host->align_buffer)
		falcon_clr_preload(virt_to_phys(host->align_buffer),
				   PAGE_ALIGN(host->align_buffer_sz) >> PAGE_SHIFT);
#endif
	if (host->adma_table)
		dma_free_coherent(mmc_dev(mmc), host->adma_table_sz,
