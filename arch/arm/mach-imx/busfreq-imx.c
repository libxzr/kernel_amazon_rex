...
volatile static int high_bus_count, med_bus_count, audio_bus_count, low_bus_count;
...
static struct delayed_work high_bus_freq_wq_request;
static struct delayed_work high_bus_freq_wq_release;
static struct delayed_work high_bus_freq_wq_release_with_timer;
static struct workqueue_struct *k_req_high_bus_freq_wk;
...
#ifndef NXP_POWER_OPTIMIZATION
		if (uart_from_osc) {
#ifdef LOW_POWER_RUN_VOLTAGE_975
			ret = regulator_set_voltage_tol(arm_reg, LOW_POWER_RUN_VOLTAGE_975, 0);
#else
...
#ifdef LOW_POWER_RUN_VOLTAGE_975
			ret = regulator_set_voltage_tol(soc_reg, LOW_POWER_RUN_VOLTAGE_975, 0);
#else
...
static void reduce_bus_freq_handler(struct work_struct *work)
{
    ...
    if (high_bus_count == 0)
	    ...
	    pr_info("high_bus_count=%d, high_bus_freq_mode=%d\n",
	        high_bus_count, high_bus_freq_mode);

	mutex_unlock(&bus_freq_mutex);
}

void request_high_bus_freq_handler(void)
{
    queue_delayed_work_on(0, k_req_high_bus_freq_wk, &high_bus_freq_wq_request, 0);
}

void release_high_bus_freq_handler(void)
{
    queue_delayed_work_on(0, k_req_high_bus_freq_wk, &high_bus_freq_wq_release, 0);
}

void release_high_bus_freq_handler_with_timer(unsigned int milisecs)
{
	pr_info("Release hi-bus-freq in %d milisecs.\n", milisecs);
	queue_delayed_work_on(0, k_req_high_bus_freq_wk, &high_bus_freq_wq_release_with_timer, msecs_to_jiffies(milisecs));
}

void _request_bus_freq(enum bus_freq_mode mode);
void _release_bus_freq(enum bus_freq_mode mode);

void request_bus_freq(enum bus_freq_mode mode)
{
    _request_bus_freq(mode);
}
EXPORT_SYMBOL(request_bus_freq);

void release_bus_freq(enum bus_freq_mode mode)
{
    _release_bus_freq(mode);
}
...
int set_low_bus_freq(void)
{
	...
	if (audio_bus_count && (low_bus_freq_mode || ultra_low_bus_freq_mode))
		...
		if (is_first_time) {
			queue_delayed_work_on(0, k_req_high_bus_freq_wk,
			                &low_bus_freq_handler, msecs_to_jiffies(1250));
		} else {
			reduce_bus_freq();
		}
...
static void high_bus_freq_request_handler(struct work_struct *work)
{
	_request_bus_freq(BUS_FREQ_HIGH);
}

static void high_bus_freq_release_handler(struct work_struct *work)
{
	_release_bus_freq(BUS_FREQ_HIGH);
}
...
static int busfreq_probe(struct platform_device *pdev)
{
	...
	/* Need workq with high priority to pull bus freq. */
	k_req_high_bus_freq_wk = alloc_workqueue("high_bus_freq", WQ_MEM_RECLAIM | WQ_HIGHPRI, 0);
	if (!k_req_high_bus_freq_wk) {
		pr_err("Creation of k_req_high_bus_freq_wk failed\n");
		return -EFAULT;
	}

	INIT_DELAYED_WORK(&bus_freq_daemon, bus_freq_daemon_handler);
	INIT_DELAYED_WORK(&low_bus_freq_handler, reduce_bus_freq_handler);
	INIT_DELAYED_WORK(&high_bus_freq_wq_request, &high_bus_freq_request_handler);
	INIT_DELAYED_WORK(&high_bus_freq_wq_release, &high_bus_freq_release_handler);
	INIT_DELAYED_WORK(&high_bus_freq_wq_release_with_timer, &high_bus_freq_release_handler);
