...
void bt_uart_switch_parent(int bt_on)
{
	...
	if (uart_from_osc) {
		while (__clk_get_enable_count(clks[IMX6SLL_CLK_UART1_SERIAL])) {
			clk_disable_unprepare(clks[IMX6SLL_CLK_UART1_SERIAL]);
			count[0]++;
		}

		while (__clk_get_enable_count(clks[IMX6SLL_CLK_UART2_SERIAL])) {
			clk_disable_unprepare(clks[IMX6SLL_CLK_UART2_SERIAL]);
			count[1]++;
		}

		while (__clk_get_enable_count(clks[IMX6SLL_CLK_UART3_SERIAL])) {
			clk_disable_unprepare(clks[IMX6SLL_CLK_UART3_SERIAL]);
			count[2]++;
		}

		while (__clk_get_enable_count(clks[IMX6SLL_CLK_UART4_SERIAL])) {
			clk_disable_unprepare(clks[IMX6SLL_CLK_UART4_SERIAL]);
			count[3]++;
		}

		while (__clk_get_enable_count(clks[IMX6SLL_CLK_UART5_SERIAL])) {
			clk_disable_unprepare(clks[IMX6SLL_CLK_UART5_SERIAL]);
			count[4]++;
		}

		if (bt_on) {
			imx_clk_set_parent(clks[IMX6SLL_CLK_UART_SEL], clks[IMX6SLL_CLK_PLL3_80M]);
			imx_clk_set_rate(clks[IMX6SLL_CLK_UART_PODF], 80000000);
		}  else {
			imx_clk_set_parent(clks[IMX6SLL_CLK_UART_SEL], clks[IMX6SLL_CLK_OSC]);
			imx_clk_set_rate(clks[IMX6SLL_CLK_UART_PODF], 4000000);
		}

		while (count[4]--)
			imx_clk_prepare_enable(clks[IMX6SLL_CLK_UART5_SERIAL]);

		while (count[3]--)
			imx_clk_prepare_enable(clks[IMX6SLL_CLK_UART4_SERIAL]);

		while (count[2]--)
			imx_clk_prepare_enable(clks[IMX6SLL_CLK_UART3_SERIAL]);

		while (count[1]--)
			imx_clk_prepare_enable(clks[IMX6SLL_CLK_UART2_SERIAL]);

		while (count[0]--)
			imx_clk_prepare_enable(clks[IMX6SLL_CLK_UART1_SERIAL]);
	}
}
