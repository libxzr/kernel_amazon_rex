...
static bool is_wan_module = false;
...
static struct cntry_locales_custom brcm_wlan_translate_custom_table[] = {
	...
	{"00", "XZ", 47},  /* Universal if Country code is unknown or empty */
	/* Do NOT add new entry above this line */

	/* FCC */
	{"US", "US", 862},  /* input ISO "US" USA WFO Sku */
	{"USW", "US", 863}, /* input ISO "US" USA WAN Sku */
	{"CA", "US", 862},  /* input ISO "CA" Canada WFO Sku */
	{"CAW", "US", 863}, /* input ISO "CA" Canada WAN Sku */
	{"MX", "US", 862},  /* input ISO "MX" Mexico WFO Sku */

	/* CE */
	{"AT", "AT", 61},   /* input ISO "AT" Austria WFO Sku */
	{"ATW", "AT", 64},  /* input ISO "AT" Austria WAN Sku */
	{"GB", "AT", 61},   /* input ISO "GB" United Kingdom WFO Sku */
	{"GBW", "AT", 64},  /* input ISO "GB" United Kingdom WAN Sku */
	{"DE", "AT", 61},   /* input ISO "DE" Germany WFO Sku */
	{"DEW", "AT", 64},  /* input ISO "DE" Germany WAN Sku */
	{"FR", "AT", 61},   /* input ISO "FR" France WFO Sku */
	{"FRW", "AT", 64},  /* input ISO "FR" France WAN Sku */
	{"IT", "AT", 61},   /* input ISO "IT" Italy WFO Sku */
	{"ITW", "AT", 64},  /* input ISO "IT" Italy WAN Sku */
	{"ES", "AT", 61},   /* input ISO "ES" Spain WFO Sku */
	{"ESW", "AT", 64},  /* input ISO "ES" Spain WAN Sku */

	{"AU", "AT", 61},   /* input ISO "AU" Australia WFO Sku */
	{"AUW", "AT", 64},  /* input ISO "AU" Australia WAN Sku */
	{"NZ", "AT", 61},   /* input ISO "NZ" New Zealand WFO Sku */
	{"NZW", "AT", 64},  /* input ISO "NZ" New Zealand WAN Sku */
	{"IN", "AT", 61},   /* input ISO "IN" India WFO Sku */
	{"INW", "AT", 64},  /* input ISO "IN" India WAN Sku */
	{"CN", "AT", 61},   /* input ISO "CN" China WFO Sku */

	{"AE", "AT", 61},   /* input ISO "AE" United Arab Emirates WFO Sku */
	{"SA", "AT", 61},   /* input ISO "SA" Kingdom of Saudi Arabia WFO Sku */
	{"EG", "AT", 61},   /* input ISO "EG" Egypt WFO Sku */

	/* Japan */
	{"JP", "JP", 125},  /* input ISO "JP" Japan WFO Sku */
	{"JPW", "JP", 126}, /* input ISO "JP" Japan WAN Sku */
...
static void *brcm_wlan_get_country_code(char *ccode)
{
	...
	memcpy(country_string, ccode,
	       min_t(int, strlen(ccode), WLC_CNTRY_BUF_SZ));
	if (is_wan_module) {
		pr_info("%s: WAN sku: cc='%s'\n", __func__, ccode);
		if (strcmp(ccode, "00") != 0) {
			country_string[2] = 'W';
			country_string[3] = '\0';
		}
	} else {
		pr_info("%s: WFO sku: cc='%s'\n", __func__, ccode);
	}

	for (i = 0; i < size; i++)
		if (strncmp(country_string, locales[i].iso_abbrev,
			    WLC_CNTRY_BUF_SZ - 1) == 0)
			return &locales[i];
	return &locales[0];
}
...
int __init brcm_init_gpio_power(struct device_node *pnode)
{
	...
	/* Check if it's a WAN module */
	if (of_get_property(pnode, "wan_module", NULL)) {
		is_wan_module = true;
		pr_info("wan_module found\n");
	}
