...
#else
static int imx_wm8960_late_init(struct snd_soc_card *card)
#endif
{
...
static int imx_wm8960_late_probe(struct snd_soc_card *card)
{
	return imx_wm8960_late_init(card);
}

static int imx_wm8960_suspend_pre(struct snd_soc_card *card)
{
	return 0;
}

static int imx_wm8960_suspend_post(struct snd_soc_card *card)
{
	return 0;
}

static int imx_wm8960_resume_pre(struct snd_soc_card *card)
{
	struct imx_wm8960_data *data = snd_soc_card_get_drvdata(card);
	int ret;
...
static int imx_wm8960_resume_post(struct snd_soc_card *card)
{
	struct imx_wm8960_data *data = snd_soc_card_get_drvdata(card);
	int ret;
...
#ifdef CONFIG_FALCON
	data->card.resume_post = imx_wm8960_resume_post;
	data->card.resume_pre = imx_wm8960_resume_pre;
	data->card.suspend_pre = imx_wm8960_suspend_pre;
	data->card.suspend_post = imx_wm8960_suspend_post;
#endif
...
#ifdef CONFIG_FALCON
static int imx_wm8960_suspend(struct platform_device *dev, pm_message_t state)
{
	return 0;
}
...
{
	struct imx_wm8960_data *data =
		(struct imx_wm8960_data *)pdev->dev.driver_data;

	if (!in_falcon())
		return 0;

	return imx_wm8960_late_init(&(data->card));
}
#endif
