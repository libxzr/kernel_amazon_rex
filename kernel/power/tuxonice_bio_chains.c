...
void dump_block_chains(void)
{
	...
	while (cur_chain) {
		...
		printk( "Chain %d (prio %d):", i, cur_chain->prio);
...
#if defined(CONFIG_LAB126)
void get_block_chain_info(u32 *data)
{
	int i = 0;
	int j;
	struct toi_bdev_info *cur_chain = prio_chain_head;
	...
	while (cur_chain) {
		...
		while (this) {
			...
			if (idx < 2) {
				...
				if (idx <= 20){
					data_tmp[idx - 2] = this->start;
					idx++;
					data_tmp[idx - 2] = this->end;
					idx++;
				}
			}
			this = this->next;
	...
	for (i = 0; i < 4; i++) {
		...
		while (cur_chain) {
			printk( " Chain %d: Extent %d. Offset %lu.\n",
					j, cur_chain->saved_state[i].extent_num,
					cur_chain->saved_state[i].offset);
			if (i == 0 && j == 0 && data[0] != cur_chain->saved_state[1].offset ) {
				data[0] = cur_chain->saved_state[1].offset;
			}

			if (idx <= 4 && ( cur_chain->saved_state[3].extent_num > cur_chain->saved_state[1].extent_num ) ) {
				if (cur_chain->saved_state[1].extent_num > 1) {
					data[1] = data_tmp[ (cur_chain->saved_state[1].extent_num - 2)*2 + 1];
				}
				data[2] = data_tmp[ (cur_chain->saved_state[3].extent_num - 2)*2 + 0 ];
				data[3] = data_tmp[ (cur_chain->saved_state[3].extent_num - 2)*2 + 1 ];
				data[4] = cur_chain->saved_state[3].extent_num;
				idx = 5;
			}
