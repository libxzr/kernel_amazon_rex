...
#if defined(CONFIG_LAB126)
/* shutdown because hibernation */
u8 is_hibernation = 0;
EXPORT_SYMBOL(is_hibernation);
u8 allow_cancel_hibernation = 0;
u8 lazy_allow_cancel_hibernation = 0;
static struct timer_list toi_timer;
u8 is_hibernation_thaw = 0;
EXPORT_SYMBOL(is_hibernation_thaw);
#endif
...
#if defined(CONFIG_LAB126)
static void toi_timer_cb( unsigned long data )
{
	release_bus_freq(BUS_FREQ_HIGH);
	printk(KERN_INFO "TuxOnIce: release bus freq\n");
}
#endif
...
#if defined(CONFIG_LAB126)
	is_hibernation = 0;
	allow_cancel_hibernation = 0;
	lazy_allow_cancel_hibernation = 0;
	is_hibernation_thaw = 0;
#endif
	free_checksum_pages();
...
#if defined(CONFIG_LAB126)
	setup_timer(&toi_timer, toi_timer_cb, 0);
#endif
	nr_hibernates++;
 ...
 *
*/
void cancel_toi_hibernation(void)
{
	if ( is_hibernation && allow_cancel_hibernation) {
		...
		printk(KERN_INFO "Hibernation cancellation is pending!\n");
	}

	if (lazy_allow_cancel_hibernation == 1) {
		lazy_allow_cancel_hibernation = 2;
	}
}
EXPORT_SYMBOL(cancel_toi_hibernation);
#endif
...
#if defined(CONFIG_LAB126)
	allow_cancel_hibernation = 0;
	lazy_allow_cancel_hibernation = 1;
#endif
...
Failed:
	...
	if (!toi_in_hibernate) {
		...
		copyback_post();
#if defined(CONFIG_LAB126)
		mod_timer(&toi_timer, jiffies + msecs_to_jiffies(5000));
#endif
		return 0;
	}
...
#if defined(CONFIG_LAB126)
	allow_cancel_hibernation = 1;

	if (lazy_allow_cancel_hibernation == 2)
