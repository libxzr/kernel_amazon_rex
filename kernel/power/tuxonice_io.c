...
#if defined(CONFIG_LAB126) 
extern void toi_meta_add_info(u32 over_sp_cnt, u32 free_page_cnt, u32 *bmap);
// #define UBOOT_START_ADDR_SP 0x9EF20E90
#define UBOOT_START_ADDR_SP 0x9EF20
static void _bitmap_set (u32 *bm, unsigned int bit)
{
	bm[bit >> 5] |= (1 << (bit & 0x1f));
}
#endif
...
static int worker_rw_loop(void *data)
{
	...
	unsigned int page_over_uboot_sp_cnt = 0;
	unsigned int hole_page_cnt = 0;
	unsigned long last_page = 0x80000;
	u32* ps1_bmap = NULL;
	if (io_write && io_pageset){
		ps1_bmap = (u32 *)kzalloc(4096*4, GFP_KERNEL);
		memset(ps1_bmap, 0x0, 4096*4);
	}
#endif
	...
	while (atomic_read(&io_count) >= atomic_read(&toi_io_workers) &&
		...
		if (io_write){
			result = write_next_page(&data_pfn, &my_io_index,
					&write_pfn);
#if defined(CONFIG_LAB126)
			if  ( (write_pfn < UBOOT_START_ADDR_SP) && (write_pfn - last_page) > 1)
				hole_page_cnt += write_pfn - last_page - 1;

			last_page = write_pfn;

			if (ps1_bmap && io_pageset &&  write_pfn >= 0x80000 && (write_pfn - 0x80000) < 131072)
				_bitmap_set(ps1_bmap, (unsigned int) write_pfn - 0x80000);

			if (io_pageset && write_pfn >= UBOOT_START_ADDR_SP)
				page_over_uboot_sp_cnt++;
#endif
		}
		else /* Reading */
...
#if defined(CONFIG_LAB126)
	if (io_write && io_pageset){
		printk("io_write=%d write_pfn=0x%X \n", io_write, write_pfn);
		printk("page_over_uboot_sp_cnt=%d hole_page_cnt=%d\n", page_over_uboot_sp_cnt, hole_page_cnt);
	}

	if (ps1_bmap && io_write && io_pageset) {
		toi_meta_add_info(page_over_uboot_sp_cnt, hole_page_cnt, ps1_bmap);
		kfree(ps1_bmap);
	}
#endif
